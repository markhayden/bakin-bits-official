...
browserTest('table fits long paths and row actions preserve navigation, target identity, and confirmation', async () => {
  ...
  try {
    ...
    // Alpha is completed: no "complete" step, cleanup available, terminate not.
    expect(await page.getByRole('menuitem', { name: 'Terminate', exact: true }).isDisabled()).toBe(true)
    await page.getByRole('menuitem', { name: 'Delete output only', exact: true }).click()
    await page.getByRole('dialog', { name: 'Delete output history?' }).getByText(/Alpha completed/).waitFor()
    await page.getByRole('button', { name: 'Cancel', exact: true }).click()
    expect(operations).toHaveLength(0)
    // Beta is exited: "Mark ended" finalizes it without seizing control.
    await page.getByRole('button', { name: 'Actions for Beta exited', exact: true }).focus()
    await page.keyboard.press('Enter')
    await page.getByRole('menuitem', { name: 'Mark ended', exact: true }).click()
    ...
    expect(operations[0]).toMatchObject({ id: 'beta', operation: 'complete' })
    expect(new URL(page.url()).pathname).toBe('/terminal')
    await openAlpha.click()
    await page.getByRole('menuitem', { name: 'Delete output only', exact: true }).click()
    ...
    await openAlpha.click()
    expect(await page.getByRole('menuitem', { name: 'Delete output only', exact: true }).isDisabled()).toBe(true)
    await page.keyboard.press('Escape')
    // Gamma is running and agent-owned: the operator can terminate directly
    // (no takeover), and Drive is offered to take over instead.
    const openGamma = page.getByRole('button', { name: 'Actions for Gamma agent-owned', exact: true })
    await openGamma.click()
    expect(await page.getByRole('menuitem', { name: 'Drive', exact: true }).isDisabled()).toBe(false)
    ...
    expect(operations.at(-1)).toMatchObject({ id: 'gamma', operation: 'terminate' })
